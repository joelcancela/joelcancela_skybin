:: ::Wakes up my PC from the same network (requires WakeOnLanC.exe).
@echo off
setlocal
...
if not defined LOCAL_AGENT (
	echo Error: LOCAL_AGENT not configured in .bin/wake.cfg. 1>&2
	exit /b 1
)
...
WakeOnLanC.exe -w -agent %LOCAL_AGENT% -mac %MAC%
